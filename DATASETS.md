...
- Annotations: `/mnt/c/ego4d_data/v2/annotations`
- Clips: `/mnt/c/ego4d_data/v2/clips`
- Config: `configs/dataset/ego4d_mq.yaml`

What it is:

- Ego4D Moments Queries is an egocentric video benchmark.
- The task is: given a high-level activity name, localize every time that activity happens in the clip.
- Officially, MQ uses a taxonomy of moment labels rather than free-form captions.

What the labels look like:

- Annotation file used here: `moments_val.json`
- Top level structure is `videos -> clips -> annotations -> labels`
- Each label entry has fields like `start_time`, `end_time`, `label`, `video_start_time`, `video_end_time`, and `primary`
- Example raw label: `take_photo_/_record_video_with_a_camera`

How this repo uses it:

- The loader groups all windows with the same label inside a clip
- It turns the label into a text query by stripping quotes and replacing `_` with spaces
- Ground truth is one or more temporal windows per query
- Metadata kept by the loader includes `clip_uid`, `video_uid`, and the original `raw_label`

Current local note:

- The local validation annotation has `329` videos, `528` clips, and `4932` label entries
...
- Annotations: `/mnt/c/ego4d_data/v2/annotations`
- Clips: `/mnt/c/ego4d_data/v2/clips`
- Config: `configs/dataset/ego4d_nlq.yaml`

What it is:

- Ego4D Natural Language Queries is an egocentric temporal grounding benchmark
- Each example starts from a free-form language query about an event in a clip
- Unlike `Ego4D MQ`, each query is paired with a single target time span

What the labels look like:

- Annotation file used here: `nlq_val.json`
- Top level structure is `videos -> clips -> annotations -> language_queries`
- Each language query includes fields such as `query`, `clip_start_sec`, and `clip_end_sec`

How this repo uses it:

- The loader uses the free-form `query` text directly
- It converts `clip_start_sec` and `clip_end_sec` into a single ground-truth window
- Metadata kept by the loader includes `clip_uid` and `video_uid`

Current local note:

- The evaluation docs describe the local validation split as `4552` queries
